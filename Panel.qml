...
Panel {
  ...
  // ---- Day notes. One JSON file, keyed by "yyyy-MM-dd", holding
  //      { title, text } per day. A dot on the cell is the only other
  //      trace — the grid stays a grid, it just remembers things now.
  property var notes: ({})
  property string editingNoteKey: ""
  property int editingNoteYear: 0
  property int editingNoteMonth: 0
  property int editingNoteDay: 0
  readonly property bool editingNote: editingNoteKey !== ""
  ...
  function close() {
    ...
    if (root.editingNote) root.closeNoteEditor()
  ...
  // ---- Day notes persistence. One JSON file under state, read at load
  //      and rewritten whole on every change — the note list is small
  //      enough that there is no reason to do anything cleverer.
  ...
  function openNoteEditor(key, year, month, day) {
    var existing = root.notes[key]
    root.editingNoteYear = year
    root.editingNoteMonth = month
    root.editingNoteDay = day
    root.editingNoteKey = key
    Qt.callLater(function() {
      noteTitleField.text = existing ? (existing.title || "") : ""
      noteBodyField.text = existing ? (existing.text || "") : ""
  ...
  function saveNoteEditor() {
    if (root.editingNoteKey === "") return
    var title = noteTitleField.text.replace(/^\s+|\s+$/g, "")
    var body = noteBodyField.text.replace(/^\s+|\s+$/g, "")
    var next = {}
    for (var k in root.notes) next[k] = root.notes[k]
    if (title === "" && body === "") delete next[root.editingNoteKey]
    else next[root.editingNoteKey] = { title: title, text: body }
    root.notes = next
    root.saveNotes()
  ...
  function deleteNoteEditor() {
    ...
    var next = {}
    for (var k in root.notes) next[k] = root.notes[k]
    delete next[root.editingNoteKey]
    root.notes = next
    root.saveNotes()
    root.closeNoteEditor()
  }
  ...
  FileView {
    id: notesFile
    path: Quickshell.env("HOME") + "/.local/state/omarchy/settings/hannes-calendar-notes.json"
    watchChanges: true
    atomicWrites: true
    printErrors: false
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      id: keyCatcher
      anchors.fill: parent
      blocked: root.editingLife || root.editingNote
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      // over a grid this quiet. A click still gets a soft
                      // hover fill so the cell reads as clickable.
                      ...
                      // Has-a-note mark. Top-right corner so it never
                      // fights the day number sitting dead center.
                      Rectangle {
                        visible: root.notes[modelData.key] !== undefined
                      ...
                      MouseArea {
                        ...
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: root.openNoteEditor(modelData.key, modelData.year, modelData.month, modelData.day)

                        PanelToolTip {
                          visible: dayMouse.containsMouse && root.notes[modelData.key] !== undefined
                          text: root.notes[modelData.key] ? root.notes[modelData.key].title || "Notiz" : ""
                          fontFamily: root.contentFontFamily
                        }
                      }
                    }
      ...
      // ---- Note editor. A scrim over the whole panel with a small card
      //      centered on it — same shape as ConfirmDialog, just with two
      //      text inputs instead of a message.
      Rectangle {
        ...
        MouseArea {
          anchors.fill: parent
          onClicked: root.closeNoteEditor()
        }
        ...
        Rectangle {
          ...
          Column {
            ...
            Text {
              textFormat: Text.PlainText
              text: root.editingNote
                ? Qt.formatDate(new Date(root.editingNoteYear, root.editingNoteMonth, root.editingNoteDay), "dddd, d MMMM yyyy")
                : ""
              color: Qt.darker(root.contentForeground, 1.4)
              font.family: root.contentFontFamily
              font.pixelSize: Style.font.bodySmall
            ...
            TextField {
              ...
              Keys.onPressed: function(event) {
                if (event.key === Qt.Key_Escape) {
                  ...
                  noteBodyField.forceActiveFocus()
                  event.accepted = true
                }
              }
            }
            ...
            Row {
              ...
              Button {
                text: "Löschen"
                visible: root.editingNote && root.notes[root.editingNoteKey] !== undefined
                foreground: root.contentForeground
                fontFamily: root.contentFontFamily
                onClicked: root.deleteNoteEditor()
              }
